...
const assembleAuthUrl = () => {
  if (client_id.value) {
    ...
    console.log('failed to assemble url');
    authUrl.value = 'http://localhost:5173/';
  }
}

watch(client_id, assembleAuthUrl);
...
<template>
    <Button v-if="!authStore.isAuthenticated" @click="handleLogin" class="flex items-center gap-2 py-6 px-6 bg-green-600 rounded-full cursor-pointer">
        Login with 
